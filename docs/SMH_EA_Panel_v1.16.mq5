...
//--- INPUT PARAMETERS ---
input string TerminalId          = "Masukan Terminal ID Anda";
...
// Conservative: naik lot tiap N layer. Bisa diubah via /setlayersper
input int    LayersPerLotIncrease   = 3;
// Lot awal entry. Bisa diubah via /setbaselot
input double BaseLot                = 0.01;
// Conservative: besar kenaikan lot. Bisa diubah via /setlotinc
...
// BASE LOT via dashboard (GV: EA_BASELOT_[TerminalId])
double ActiveBaseLot = 0.01;
string BaseLotVarName = "";

// LAYERS PER LOT (conservative) via dashboard (GV: EA_LAYERSPER_[TerminalId])
int    ActiveLayersPerLot = 3;
string LayersPerVarName   = "";

// LOT INCREMENT (conservative) via dashboard (GV: EA_LOTINC_[TerminalId])
double ActiveLotIncrement = 0.01;
...
void SyncLotMultiplier()
{
   if(GlobalVariableCheck(MultVarName))
   {
      ...
      if(gv > 0.0 && gv != ActiveLotMultiplier)
      {
...
void SyncBaseLot()
{
   if(GlobalVariableCheck(BaseLotVarName))
   {
      double gv = GlobalVariableGet(BaseLotVarName);
      if(gv > 0 && gv != ActiveBaseLot)
      {
         ActiveBaseLot = gv;
         Print("[PARAM] BaseLot: ", DoubleToString(ActiveBaseLot,2));
      }
   }
   else
   {
      ActiveBaseLot = BaseLot;
      GlobalVariableSet(BaseLotVarName, ActiveBaseLot);
   }
}

void SyncLayersPerLot()
{
   if(GlobalVariableCheck(LayersPerVarName))
   {
      int gv = (int)GlobalVariableGet(LayersPerVarName);
      if(gv > 0 && gv != ActiveLayersPerLot)
      {
         ActiveLayersPerLot = gv;
         Print("[PARAM] LayersPerLot: ", ActiveLayersPerLot);
      }
   }
   else
   {
      ActiveLayersPerLot = LayersPerLotIncrease;
      if(ActiveLayersPerLot < 1) ActiveLayersPerLot = 1;
      GlobalVariableSet(LayersPerVarName, (double)ActiveLayersPerLot);
   }
}

void SyncLotIncrement()
{
   if(GlobalVariableCheck(LotIncVarName))
   {
      double gv = GlobalVariableGet(LotIncVarName);
      if(gv > 0 && gv != ActiveLotIncrement)
      {
         ActiveLotIncrement = gv;
         Print("[PARAM] LotIncrement: ", DoubleToString(ActiveLotIncrement,2));
      }
   }
   else
   {
      ActiveLotIncrement = LotIncrement;
      if(ActiveLotIncrement <= 0) ActiveLotIncrement = 0.01;
      GlobalVariableSet(LotIncVarName, ActiveLotIncrement);
   }
}
...
double CalculateLotForLayer(int layerNumber)
{
   ...
   {
      int grp = (int)((layerNumber - 1) / ActiveLayersPerLot);
      lots = ActiveBaseLot + (grp * ActiveLotIncrement);
   }
   else
      lots = ActiveBaseLot * MathPow(ActiveLotMultiplier, layerNumber - 1);
...
int GetLotGroup(int layerNumber) { return (int)MathCeil((double)layerNumber / ActiveLayersPerLot); }
   ...
   for(int i = PositionsTotal() - 1; i >= 0; i--)
   {
      if(posInfo.SelectByIndex(i))
      {
            ...
            posInfo.PositionType() == POSITION_TYPE_BUY)
         {
            ...
            if(ActiveMartingaleMode == MODE_CONSERVATIVE)
            {
               grp = (int)MathRound((vol - ActiveBaseLot) / ActiveLotIncrement) + 1;
               if(grp < 1) grp = 1;
            }
            else
            {
               grp = 1;
               double testLot = ActiveBaseLot;
               for(int g = 1; g <= 50; g++)
               {
                  ...
                  testLot = ActiveBaseLot * MathPow(ActiveLotMultiplier, g);
               }
            }
...
void CheckReentry()
{
   ...
   if(ReentryDirection == 0)
   {
      ...
      bool bOK = trade.Buy(ActiveBaseLot, _Symbol, 0, 0, 0, BuildComment(Comment_Reentry_Buy)); Sleep(50);
      bool sOK = trade.Sell(ActiveBaseLot, _Symbol, 0, 0, 0, BuildComment(Comment_Reentry_Sell));
   ...
   if(ReentryDirection == 1)
   {
      ...
      if(trade.Buy(ActiveBaseLot,_Symbol,0,0,0,BuildComment(Comment_Reentry_Buy))) { Print("[REENTRY] BUY"); WaitingForReentry=false; ReentryDirection=0; }
   ...
   if(ReentryDirection == -1)
   {
      ...
      if(trade.Sell(ActiveBaseLot,_Symbol,0,0,0,BuildComment(Comment_Reentry_Sell))) { Print("[REENTRY] SELL"); WaitingForReentry=false; ReentryDirection=0; }
   ...
   if(ReentryDirection == 2)
   {
      ...
      if(sig == 1)
      {
         if(trade.Buy(ActiveBaseLot,_Symbol,0,0,0,BuildComment(Comment_Reentry_Buy)))
      ...
      {
         if(trade.Sell(ActiveBaseLot,_Symbol,0,0,0,BuildComment(Comment_Reentry_Sell)))
...
void CheckInitialEntry()
{
   ...
   {
      bool bOK=trade.Buy(ActiveBaseLot,_Symbol,0,0,0,BuildComment(Comment_Buy)); Sleep(50);
      bool sOK=trade.Sell(ActiveBaseLot,_Symbol,0,0,0,BuildComment(Comment_Sell));
   ...
   else
   {
      ...
      if(sig==1)
      {
         if(trade.Buy(ActiveBaseLot,_Symbol,0,0,0,BuildComment(Comment_Buy))) Print("[ENTRY ONE_WAY] BUY @ ",ask);
         else LogTradeFail("ENTRY ONE_WAY BUY");
      }
      else if(sig==-1)
      {
         if(trade.Sell(ActiveBaseLot,_Symbol,0,0,0,BuildComment(Comment_Sell))) Print("[ENTRY ONE_WAY] SELL @ ",bid);
...
double GetNextLotFromPositions(ENUM_POSITION_TYPE posType)
{
   if(ActiveMartingaleMode == MODE_AGGRESSIVE)
   {
      ...
      double nextLot = (deepestLot > 0) ? deepestLot * ActiveLotMultiplier : ActiveBaseLot;
   ...
   // Conservative: baca lot terbesar dari posisi, naik setelah ActiveLayersPerLot
   double dL=0;
   for(int i=PositionsTotal()-1;i>=0;i--)
      if(posInfo.SelectByIndex(i) && posInfo.Symbol()==_Symbol && posInfo.Magic()==InpMagic && posInfo.PositionType()==posType)
         if(posInfo.Volume()>dL) dL=posInfo.Volume();
   if(dL<=0) return ActiveBaseLot;
   int cnt=0;
   for(int i=PositionsTotal()-1;i>=0;i--)
      if(posInfo.SelectByIndex(i) && posInfo.Symbol()==_Symbol && posInfo.Magic()==InpMagic && posInfo.PositionType()==posType)
         if(MathAbs(posInfo.Volume()-dL)<0.0001) cnt++;
   double nl=(cnt>=ActiveLayersPerLot)?dL+ActiveLotIncrement:dL;
   double minL=SymbolInfoDouble(_Symbol,SYMBOL_VOLUME_MIN),maxL=SymbolInfoDouble(_Symbol,SYMBOL_VOLUME_MAX),stepL=SymbolInfoDouble(_Symbol,SYMBOL_VOLUME_STEP);
...
void CheckBuyMartingale()
{
   ...
   if(BUY_LastEntry - ask >= ConvertPointsToPrice(ActiveLayerDistance))
   {
      ...
      if(cs>=ActiveLayersPerLot){lots+=ActiveLotIncrement;Print("[WARN] BUY safeguard: lot->",DoubleToString(lots,2));}
      if(ActiveMaxLot>0&&lots>ActiveMaxLot)lots=ActiveMaxLot;
...
void CheckSellMartingale()
{
   ...
   if(bid - SELL_LastEntry >= ConvertPointsToPrice(ActiveLayerDistance))
   {
      ...
      for(int i=PositionsTotal()-1;i>=0;i--)
         if(posInfo.SelectByIndex(i)&&posInfo.Symbol()==_Symbol&&posInfo.Magic()==InpMagic&&posInfo.PositionType()==POSITION_TYPE_SELL)
            if(MathAbs(posInfo.Volume()-lots)<0.0001) cs++;
      if(cs>=ActiveLayersPerLot){lots+=ActiveLotIncrement;Print("[WARN] SELL safeguard: lot->",DoubleToString(lots,2));}
      if(ActiveMaxLot>0&&lots>ActiveMaxLot)lots=ActiveMaxLot;
...
void CheckContinuousHedging()
{
   ...
   if(BUY_LayerCount>=Hedging_Start_Layer && SELL_LayerCount==0)
   {
      ...
      if(needH)
      {
         if(trade.Sell(ActiveBaseLot,_Symbol,0,0,0,BuildComment(Comment_Sell)))
   ...
   if(SELL_LayerCount>=Hedging_Start_Layer && BUY_LayerCount==0)
   {
      ...
      if(needH)
      {
         if(trade.Buy(ActiveBaseLot,_Symbol,0,0,0,BuildComment(Comment_Buy)))
...
int OnInit()
{
   ...
   Print("========================================");
   Print("SMART EA Panel v1.16");
   Print("Terminal / ID Trading   : ", TerminalId, " | Account: ", Account_ID);
   ...
   Print("Base Lot   : ", DoubleToString(ActiveBaseLot,2), " | Layers/Lot: ", ActiveLayersPerLot, " | LotInc: ", DoubleToString(ActiveLotIncrement,2));
   ...
   Print("Control    : SMH_EA_Controller_v1.16");
   Print("========================================");
...
void OnTick()
{
   ...
   SyncMaxLot(); SyncBaseLot(); SyncLayersPerLot(); SyncLotIncrement(); SyncMaxLayer();
   SyncTradingHours();
   SyncLicenceExpiry();
